...
<div class="easyui-layout" data-options="fit:true">
    ...
    <div id="wu-toolbar">
        <div class="wu-toolbar-button">
            <a href="#" class="easyui-linkbutton" iconCls="icon-add" onclick="openAdd()" plain="true">添加入库单</a>
            <a href="#" class="easyui-linkbutton" iconCls="icon-cart-edit" onclick="openEdit()" plain="true">入库</a>
            <a href="#" class="easyui-linkbutton" iconCls="icon-eye" onclick="openView()" plain="true">查看入库单</a>
        </div>
        <div class="wu-toolbar-search">
            ...
            <select id="search-payType" class="easyui-combobox" panelHeight="auto" style="width:120px">
            	...
            	<option value="4">E区</option>
            	<option value="5">F区</option>
            </select>
            <label>状态:</label>
...
<form id="add-form" method="post">
        <table>
            <tr>
                <td  width="60px" align="right">存放区域:</td>
                <td>
                	<select name="payType" id="add-payType" class="easyui-combobox" panelHeight="auto" style="width:500px" data-options="required:true, missingMessage:'请选择支付方式'">
		            	<option value="0" selected>A区</option>
            			<option value="1">B区</option>
            			<option value="2">C区</option>
            			<option value="3">D区</option>
            			<option value="4">E区</option>
            ...
            <tr>
            	<td align="right">关联采购单:</td>
            	<td>
            	<textarea name="stockinid" id="associate-id" rows="1" class="wu-textarea" style="width:490px"></textarea>
            	</td>
            	<td>
...
<div id="associate-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'" style="width:750px; padding:10px;">
	<table id="show-purchasein-datagrid" class="easyui-datagrid" style="width:716px;height:450px">
	</table>
</div>
...
function add(){
	...
	$.ajax({
		...
		dataType:'json',
		type:'post',
		data:{productList:JSON.stringify(selectedProducts),remark:$("#add-remark").val(),payType:$("#add-payType").combobox('getValue'),status:$("#add-status").combobox('getValue'),id:$("#ordernum").val(),purchaseInId:$("#associate-id").val()},
...
function selectProduct(){
	...
	function openView(){
		...
		$('#view-dialog').dialog({
            ...
            onBeforeOpen:function(){
            	...
            	var productList = item.stockInDetailList;
	...
	*/
	function openAssociate(){
		...
		$('#associate-dialog').dialog({
            ...
            buttons: [{
                ...
                iconCls: 'icon-ok',
                handler: function(){
                	var item = $('#show-purchasein-datagrid').datagrid('getSelections');
    				if(item == null || item.length == 0){
    					$.messager.alert('信息提示','请至少选择一个订货单！','info');
    					return;
    				}
    				var productLists = item[0].purchaseInDetailList;
            ...
            onBeforeOpen:function(){
            	$('#show-purchasein-datagrid').datagrid({
	...
	function openEdit(){
		...
		if(item.status == 1){
			$.messager.alert('信息提示','该入库单已入库！','info');
			return;
		}
		console.log(JSON.stringify(item.stockInDetailList));
		$.ajax({
			url:'../stock_in/edit',
			dataType:'json',
			type:'post',
			data:{stockDetailList:JSON.stringify(item.stockInDetailList),id:item.id},
			success:function(data){
				if(data.type == 'success'){
	...
	*/
	$('#data-datagrid').datagrid({
		...
		columns:[[
			{ field:'chk',checkbox:true},
			{ field:'id',title:'入库单号',width:100,sortable:true},
			{ field:'purchaseInId',title:'关联采购单号',width:100,sortable:true},
			{ field:'money',title:'金额',width:100,sortable:true},
			{ field:'payType',title:'存放区域',width:100,formatter:function(value,index,row){
				switch(value){
					case 0:return 'A区';
					case 1:return 'B区';
					case 2:return 'C区';
			...
			{ field:'operator',title:'操作人',width:100,sortable:true},
			{ field:'remark',title:'商品描述',width:200,sortable:true},
			{ field:'createTime',title:'入库时间',width:200,formatter:function(value,index,row){return format(value);}}
		]]
	});
	function GetDateNow(){
		// 时间戳
		...
		var str = String('RK'+year + mouth + day + hours + minutes + seconds)
		return str;
	}
