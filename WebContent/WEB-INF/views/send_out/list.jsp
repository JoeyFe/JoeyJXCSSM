...
<div class="easyui-layout" data-options="fit:true">
    ...
    <div id="wu-toolbar">
        <div class="wu-toolbar-button">
            <a href="#" class="easyui-linkbutton" iconCls="icon-add" onclick="openAdd()" plain="true">添加发货单</a>
            <a href="#" class="easyui-linkbutton" iconCls="icon-cart-edit" onclick="openEdit()" plain="true">发货</a>
            <a href="#" class="easyui-linkbutton" iconCls="icon-eye" onclick="openView()" plain="true">查看发货详单</a>
        </div>
               ...
               <tr>
                ...
                <td>
                	<select name="status" id="add-status" class="easyui-combobox" panelHeight="auto" style="width:500px" data-options="required:true, missingMessage:'请选择状态'">
		            	<option value="0" selected>不发货</option>
		            	<option value="1">发货</option>
		            </select>
                </td>
            </tr>
            <tr>
            	<td align="right">关联出库单:</td>
            	<td>
...
<div id="associate-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'" style="width:750px; padding:10px;">
	<table id="show-deliver-datagrid" class="easyui-datagrid" style="width:716px;height:450px">
	</table>
</div>
...
function add(){
	...
	if(selectedProducts.length <= 0){
		$.messager.alert('信息提示','请至少选择一个商品进行发货!','warning');
		return;
	}
	...
	//发货单前端，读取表单数据通过ajax异步传输到controller层
	$.ajax({
		url:'../send_out/add',
		dataType:'json',
		type:'post',
		data:{productList:JSON.stringify(selectedProducts),remark:$("#add-remark").val(),payType:$("#add-payType").combobox('getValue'),status:$("#add-status").combobox('getValue'),id:$("#ordernum").val(),deliverOutId:$("#associate-id").val(),customId:$("#customid").val(),customPhone:$("#customphone").val(),customAddr:$("#customaddr").val()},
...
function selectProduct(){
	...
	*/
	function openAssociate(){
		...
		$('#associate-dialog').dialog({
            ...
            buttons: [{
                ...
                iconCls: 'icon-ok',
                handler: function(){
                	var item = $('#show-deliver-datagrid').datagrid('getSelections');
    				if(item == null || item.length == 0){
    					$.messager.alert('信息提示','请至少选择一个订货单！','info');
    					return;
    				}
    				var productLists = item[0].deliverOutDetailList;
            ...
            onBeforeOpen:function(){
            	$('#show-deliver-datagrid').datagrid({
            		...
            		columns:[[
            			...
            			{ field:'customId',title:'顾客编号',width:100,sortable:true},
            			{ field:'productNum',title:'商品数量',width:100,sortable:true},
	...
	function openEdit(){
		...
		if(item.status == 1){
			$.messager.alert('信息提示','该发货单已发货！','info');
			return;
		}
		console.log(JSON.stringify(item.sendOutDetailList));
		$.ajax({
			url:'../send_out/edit',
			dataType:'json',
			type:'post',
			data:{sendDetailList:JSON.stringify(item.sendOutDetailList),id:item.id},
			success:function(data){
				if(data.type == 'success'){
					$.messager.alert('信息提示','发货成功！','info');
					//$("#add-content").val('');
					$('#data-datagrid').datagrid('reload');
				}else{
					$.messager.alert('信息提示',data.msg,'warning');
	...
	*/
	$('#data-datagrid').datagrid({
		...
		columns:[[
			{ field:'chk',checkbox:true},
			{ field:'id',title:'发货单号',width:100,sortable:true},
			{ field:'deliverOutId',title:'关联出库单号',width:100,sortable:true},
			{ field:'money',title:'金额',width:100,sortable:true},
			{ field:'payType',title:'快递公司',width:100,formatter:function(value,index,row){
				switch(value){
					case 0:return '顺丰';
			...
			{ field:'operator',title:'操作人',width:100,sortable:true},
			{ field:'remark',title:'商品描述',width:200,sortable:true},
			{ field:'createTime',title:'发货时间',width:200,formatter:function(value,index,row){return format(value);}}
		]]
	});
	function GetDateNow(){
		// 时间戳
		...
		var str = String('FH'+year + mouth + day + hours + minutes + seconds)
		return str;
	}
