...
<div class="easyui-layout" data-options="fit:true">
    ...
    <div id="wu-toolbar">
        <div class="wu-toolbar-button">
            <a href="#" class="easyui-linkbutton" iconCls="icon-add" onclick="openAdd()" plain="true">添加退货单</a>
            <a href="#" class="easyui-linkbutton" iconCls="icon-cart-edit" onclick="openEdit()" plain="true">退货已到货</a>
            <a href="#" class="easyui-linkbutton" iconCls="icon-eye" onclick="openView()" plain="true">查看退货详单</a>
        </div>
               ...
               <tr>
                ...
                <td>
                	<select name="status" id="add-status" class="easyui-combobox" panelHeight="auto" style="width:500px" data-options="required:true, missingMessage:'请选择状态'">
		            	<option value="0" selected>未退回</option>
		            	<option value="1">已退回</option>
		            </select>
                </td>
            </tr>
...
function add(){
	...
	if(selectedProducts.length <= 0){
		$.messager.alert('信息提示','请至少选择一个商品进行退货!','warning');
		return;
	}
	...
	$.ajax({
		...
		dataType:'json',
		type:'post',
		data:{productList:JSON.stringify(selectedProducts),remark:$("#add-remark").val(),payType:$("#add-payType").combobox('getValue'),status:$("#add-status").combobox('getValue'),id:$("#ordernum").val(),sellOutId:$("#associate-id").val()},
...
function selectProduct(){
	...
	*/
	...
	function openEdit(){
		...
		if(item.status == 1){
			$.messager.alert('信息提示','该退货单已退货！','info');
			return;
		}
		...
		$.ajax({
			...
			dataType:'json',
			type:'post',
			data:{sellDetailList:JSON.stringify(item.sellBackDetailList),id:item.id},
			success:function(data){
				if(data.type == 'success'){
					$.messager.alert('信息提示','退回成功！','info');
					//$("#add-content").val('');
					$('#data-datagrid').datagrid('reload');
				}else{
					$.messager.alert('信息提示',data.msg,'warning');
	...
	*/
	$('#data-datagrid').datagrid({
		...
		columns:[[
			{ field:'chk',checkbox:true},
			{ field:'id',title:'退货单号',width:100,sortable:true},
			...
			{ field:'operator',title:'操作人',width:100,sortable:true},
			{ field:'remark',title:'商品描述',width:200,sortable:true},
			{ field:'createTime',title:'退回时间',width:200,formatter:function(value,index,row){return format(value);}}
		]]
	});
	function GetDateNow(){
		// 时间戳
		...
		var str = String('TH'+year + mouth + day + hours + minutes + seconds)
		return str;
	}
